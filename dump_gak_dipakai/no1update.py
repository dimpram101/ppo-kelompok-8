...
def refreshTable(basics = False, ratios = True):
  table.clear()
  table.field_names = ["z", "x1", "x2", "s1", "s2", "a", "rhs", "ratio"]
  for x in b:
    table.add_row(x)
  ...
  if ratios == False:
    table.clear()
    table.field_names = ["z", "x1", "x2", "s1", "s2", "a", "rhs"]
    for x in b:
      table.add_row(x[:-1])
...
class SimplexMethod:
  ...
  def findMinValue(self):
    ...
    for i in range(len(self.b)):
      if minValue in self.b[i]:
        index = i
        break

    unsurKunci = 0
    temp =[]
    for i in range(len(self.b[index][:6])):
      if self.b[index][i] != 0:
        temp.append(-1 * self.b[index][i])
      # print(self.b[index][i])
    unsurKunci = min(temp)
